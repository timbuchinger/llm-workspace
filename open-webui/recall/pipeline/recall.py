...
class Pipeline:
    ...
    async def inlet(self, body: dict, user: Optional[dict] = None) -> dict:
        ...
        response = requests.post(
            "http://recall-svc:8000/search-similar",
            json={
                "query": user_message,
                "collections": list(get_collections(user)),
        ...
        if response.status_code == 200:
            ...
            if documents:
                # Read the prompt template from the same directory as this file
                ...
                # Read the no-results prompt template
                current_dir = os.path.dirname(os.path.abspath(__file__))
